...
apiProvider.load = function () {
  if (process.env.NODE_ENV === 'production') {
    ...
    apiProvider.apiConfig['CESHI_API_HOST_LOG'] = window.API.CESHI_API_HOST_LOG
  } else {
...
apiProvider.mapModuleRoute = function (module = 'CESHI_API_HOST') {
  if (module) {
    ...
    } else if (module === 'CESHI_API_HOST_LOG') {
...
apiProvider.getApiAddress = function (url, module) {
  ...
  if (url) {
    if (url.indexOf('/') === 0) {
      // apiAddress = process.env.NODE_ENV === 'production' ? url : this.mapModuleRoute(module) + url
