...
gateway_config="gateway_test.yml"
if [ "$1" == "gateway_pro" ]; then
    gateway_config="gateway_pro.yml"
fi

if [ -n "$3" ]; then
    echo "start modify spring.application.name, modify before:"
    sed -n '/^.*name: .*GATEWAY-CORE/,0p' ../config/$gateway_config
    systemType=`uname`
    echo "system type: "$systemType
    if [ "$systemType" == "Darwin" ]; then
        sed -i '' '/^.*name/s/:.*GATEWAY-CORE/: '$3'-GATEWAY-CORE/g' ../config/$gateway_config
    else
        sed -i '/^.*name/s/:.*GATEWAY-CORE/: '$3'-GATEWAY-CORE/g' ../config/$gateway_config
    fi
    echo "modify after:"
...
if [ "$proc_watcher" == "yes" ]; then
    ...
    echo "*/1 * * * * sh $working_directory/gw_proc_watcher.sh \"$1\" \"$working_directory\" \"./run.sh --no-watch $1 $2 \"  >/dev/null 2>&1" >> $cronfile
    crontab $cronfile
    rm $cronfile
    exit 0
fi
