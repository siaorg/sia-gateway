...
gateway_config="gateway_synchspeed_test.yml"
if [ "$1" == "gateway_synchspeed_pro" ]; then
    gateway_config="gateway_synchspeed_pro.yml"
fi
...
if [ "$proc_watcher" == "yes" ]; then
    ...
    echo "*/1 * * * * sh $working_directory/gw_proc_watcher.sh \"$1\" \"$working_directory\" \"./run4synchspeed.sh --no-watch $1 $2 \"  >/dev/null 2>&1" >> $cronfile
    crontab $cronfile
    rm $cronfile
    exit 0
fi
